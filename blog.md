...
ISUNARABE 合同演習 2026 に team-7 として参加した。言語は Rust (Axum + Tokio)、3台の c5.large (2vCPU) を使い、最終スコアは **21,087,000**。初期スコア 38,300 からの **553倍** まで引き上げた。

競合チームのハイスコアは 21,372,000。差はわずか 285,000 (約1.3%) で、惜しくも届かなかった。

本記事では、約13時間・74コミットに及ぶチューニングの全過程を、失敗も含めて振り返る。

## チーム構成と AI 活用

今回の最大の特徴は、**全作業を Claude Code (Opus 4.6) が主導した**こと。人間（自分）はレビューと方針決定に徹し、コーディング・分析・デプロイ操作のほぼすべてを AI が行った。
...
すべての作業は **2026年5月9日 12:38 から 5月10日 01:51** の約13時間で行われた。
...
| 19:10 - 19:35 | 最終最適化 | ahash, manual datetime, zero-alloc |
| 01:51 | 最終コミット | cache pre-warm, UUID defer |
